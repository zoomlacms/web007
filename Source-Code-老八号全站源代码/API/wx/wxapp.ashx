...
public class wxapp : API_Base, IHttpHandler
{
    ...
    public string siteUrl = "https://a013.2018.z01.com";
    //---------------Extend
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            switch (Action)
            {
                ...
                case "node_list"://分类节点信息(首页只显示第二级节点)
                    {
                        ...
                        string nid = DataConvert.CStr(Req("nid"));
                        string where = "1=1 ";
                        switch (ntype)
                        {
                            case "product":
                                where += " AND NodeType IN (2) ";
                                break;
                            case "content":
                            default:
                                where += " AND NodeType IN (1)";
                                break;
                        }
                        if (!H_IsEmpty(nid))
                        {
                            SafeSC.CheckIDSEx(nid);
                            where += " AND NodeID IN (" + nid + ")";
                        }
                        DataTable dt = DBCenter.SelWithField("ZL_Node", "NodeID,NodeName", where);
                ...
                case "swiper_list"://获取首页滑动幻灯片
                    {
                        ...
                        setting.where = "A.EliteLevel=1 AND A.Status=99 AND NodeID IN (2)";//42
                        ...
                        for (int i = 0; i < setting.dt.Rows.Count; i++)
                        {
                            ...
                            dr["Pic"] = H_ImageUrl(DataConvert.CStr(dr["Pic"]));
                        }
                        retMod.retcode = M_APIResult.Success;
                ...
                case "content_get"://单条内容获取,用于显示内容详情
                    {
                        ...
                        string fields = "TOP 1 A.title,A.generalId,A.createTime,A.inputer,A.nodeId,A.modelId,B.*";
                        string tbName = DataConvert.CStr(DBCenter.ExecuteScala("ZL_CommonModel", "TableName", "GeneralID=" + id));
                        DataTable dt = DBCenter.JoinQuery(fields, "ZL_CommonModel", tbName, "A.ItemID=B.ID", "A.GeneralID=" + id);
                        if (dt.Rows.Count > 0)
                        {
                            DataRow dr = dt.Rows[0];
                            if (dt.Columns.Contains("pic")) { dr["pic"] = H_ImageUrl(DataConvert.CStr(dr["pic"])); }
                            //if (dt.Columns.Contains("images")) { dr["images"] = H_ImageUrlInContent(DataConvert.CStr(dr["images"])); }
                            if (dt.Columns.Contains("content")) { dr["content"] = H_ImageUrlInContent(DataConvert.CStr(dr["content"])); }
                        }
                        retMod.retcode = M_APIResult.Success;
                        retMod.result = JsonConvert.SerializeObject(dt);
                    }
                ...
                case "index_tops"://首页推荐滚动图与商品
                    {
                        ...
                        if (nid > 0)
                        {
                            where += " AND NodeID IN (" + H_GetNodeIDS(nid) + ")";
                        }
                        DataTable dt = DBCenter.SelTop(4, "ID", fields, "ZL_Commodities", where, "ID DESC");
                ...
                case "product_list"://根据节点等条件获取商品列表
                    {
                        ...
                        {
                            setting.where = "NodeID IN (" + H_GetNodeIDS(nid) + ")";
                        }
                        else { setting.where = " 1=1 "; }
                        dt = DBCenter.SelPage(setting);
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            dt.Rows[i]["Clearimg"] = H_ImageUrl(DataConvert.CStr(dt.Rows[i]["Clearimg"]));
                        }
                        retMod.retcode = M_APIResult.Success;
                        retMod.result = JsonConvert.SerializeObject(dt);
                        retMod.page = new M_API_Page(setting);
                ...
                case "product_get"://商品详情信息
                    {
                        ...
                        proMod.Clearimg = H_ImageUrl(proMod.Clearimg);
                        retMod.retcode = M_APIResult.Success;
                ...
                case "user_getlast"://用户获取本身的最新信息
                    {
                        ...
                        M_LiteUser litemu = liteBll.SelReturnModel(uid);
                        retMod.retcode = M_APIResult.Success;
                ...
                case "user_login_openid"://用户使用微信的openid登录,仅用于教练登录
                    {
                        ...
                        string openid = GetOpenIdByCode(code,2);
                ...
                case "user_sync"://根据openid,用户自动注册或登录
                    {
                        ...
                        string code = DataConvert.CStr(Req("code"));
                        string openid = GetOpenIdByCode(code,1);
                        if (string.IsNullOrEmpty(openid)) { throw new Exception("用户授权失效"); }
                        ...
                        else
                        {
                            ...
                            //----------------------------
                            M_LiteUser litemu = liteBll.SelReturnModel(newmu.UserID);
                            retMod.retcode = M_APIResult.Success;
                            retMod.result = JsonConvert.SerializeObject(litemu);
                        }
                ...
                case "user_code"://根据code更新绑定openid
                    {
                        ...
                        try
                        {
                            string openid = GetOpenIdByCode(code, DataConvert.CLng(Req("appId"), 1));
                ...
                case "user_login_passwd"://通过用户名与密码登录
                    {
                        ...
                        {
                            M_LiteUser litemu = liteBll.SelModelByPasswd(name, passwd);
                ...
                case "user_update"://更新用户信息
                    {
                        ...
                        M_LiteUser litemu = JsonConvert.DeserializeObject<M_LiteUser>(Req("mu"));
                        //检测手机号或身份证号不可重复,允许为空
                        if (!string.IsNullOrEmpty(litemu.mobile) && !litemu.mobile.Equals(basemu.Mobile))
                        {
                            List<SqlParameter> sp = new List<SqlParameter>()
                            { new SqlParameter("mobile",litemu.mobile)};
                            if (DBCenter.IsExist("ZL_UserBase", "mobile=@mobile", sp))
                            {
                                throw new Exception("手机号已存在,请校验号码是否正确");
                            }
                        }
                        if (!string.IsNullOrEmpty(litemu.idcard) && !litemu.idcard.Equals(basemu.IDCard))
                        {
                            List<SqlParameter> sp = new List<SqlParameter>()
                            { new SqlParameter("idcard",litemu.idcard)};
                            if (DBCenter.IsExist("ZL_UserBase", "idcard=@idcard", sp))
                            {
                                throw new Exception("证件号已存在,请校验号码是否正确");
                            }
                        }
                        mu.HoneyName = litemu.honeyName;
                        mu.UserFace = litemu.userFace;
                        buser.UpdateByID(mu);
                        basemu.Mobile = litemu.mobile;
                        basemu.CardType = litemu.cardType;
                        basemu.IDCard = litemu.idcard;
                        basemu.UserSex = litemu.sex == "男" ? true : false;
                        basemu.Address = litemu.address;
                        basemu.Bugle = litemu.age;
                        if (basemu.UserId == 0)
                        {
                        ...
                        //----------------
                        litemu = liteBll.SelReturnModel(uid);
                        retMod.retcode = M_APIResult.Success;
                        retMod.result = JsonConvert.SerializeObject(litemu);
                    }
                    break;
                ...
                case "cart_add"://增加或减少购物车数量
                    {
                        ...
                        M_Cart cartMod = cartBll.SelModelByWhere(mu.UserID, proID);
                        if (cartMod == null)
                        {
                            cartMod = new M_Cart();
                ...
                case "cart_list":
                    {
                        DataTable dt = SelByCartID(mu.UserID);
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            dt.Rows[i]["proimg"] = H_ImageUrl(DataConvert.CStr(dt.Rows[i]["proimg"]));
                        }
                        retMod.retcode = M_APIResult.Success;
                        retMod.result = JsonConvert.SerializeObject(dt);
                    }
                ...
                case "order_list":
                    {
                        ...
                        int uid = DataConvert.CLng(Req("uid"));
                        //教练查询自己的推荐学员订单
                        int puid = DataConvert.CLng(Req("puid"));
                        string where = "OrderStatus>=0 ";//
                        if (uid > 0) { where += " AND UserID=" + uid; }
                        if (puid > 0) { where += " AND UserID IN (SELECT UserID FROM ZL_User WHERE ParentUserID=" + puid + ")"; }
                        PageSetting setting = new PageSetting()
                        {
                            cpage = cpage,
                            t1 = "ZL_OrderInfo",
            ...
            case "payment_checkout"://生成支付参数给客户端
                    {
                        #region 支付
                        M_WX_APPID appMod = new B_WX_APPID().SelReturnModel(1);
    ...
    //对内容中的图片路径进行处理
    public string H_ImageUrlInContent(string content)
    {
        return content.Replace("/UploadFiles/", siteUrl + "/UploadFiles/");
    }
    //判断字符串是否为空
    public bool H_IsEmpty(string text)
    {
        return (string.IsNullOrEmpty(text) || text.Equals("undefined"));
    }
    //为图片链接加上域名等信息
    public string H_ImageUrl(string url)
    {
    ...
    //获取指定节点下的所有子节点IDS
    public string H_GetNodeIDS(int nid)
    {
        string nids = "";
